...
import { generateQuestionFromEdital, getAIExplanation } from '../services/geminiService';
import { saveResult } from '../services/statsService';
import { supabase } from '../services/supabaseClient';
import { SmartToyIcon, CheckCircleIcon, CloseIcon } from './icons';

const AiTutorModal: React.FC<{ explanation: AiExplanation | null; onClose: () => void }> = ({ explanation, onClose }) => {
...
const SubjectSelector: React.FC<{ onSelect: (subject: 'LÍNGUA PORTUGUESA' | 'MATEMÁTICA') => void }> = ({ onSelect }) => (
...
const QuestionArena: React.FC = () => {
    const [selectedSubject, setSelectedSubject] = useState<'LÍNGUA PORTUGUESA' | 'MATEMÁTICA' | null>(null);
    ...
    const loadNewQuestion = useCallback(async (subject: 'LÍNGUA PORTUGUESA' | 'MATEMÁTICA') => {
        ...
        try {
            const newQuestion = await generateQuestionFromEdital(subject);
            if (newQuestion) {
                ...
                throw new Error("Não foi possível gerar uma nova questão.");
            }
        } catch (e) {
            setError("Ocorreu um erro ao buscar a questão. Tente novamente.");
            console.error(e);
        } finally {
    ...
    const handleAnswer = async (optionIndex: number) => {
        ...
        if (!correct) {
            ...
            setTimeout(() => loadNewQuestion(selectedSubject!), 1000);
        }
    };

    const handleSubjectSelect = (subject: 'LÍNGUA PORTUGUESA' | 'MATEMÁTICA') => {
        setQuestions([]);
        setQuestionIndex(0);
    ...
    if (isLoadingQuestion && questions.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center h-full text-center p-6">
                ...
                <h2 className="mt-4 text-lg font-semibold text-slate-700 dark:text-slate-300">Gerando um novo desafio...</h2>
                <p className="text-sm text-slate-500">Aguarde, o Método do Pai está preparando uma questão fresquinha para você!</p>
            </div>
        );
    }
        ...
        <div className="relative h-full flex flex-col w-full max-w-3xl mx-auto">
            <header className="px-6 py-4 flex flex-col gap-3">
                ...
                <div className="flex justify-between items-center">
                    <span className="text-sm font-bold text-slate-400">QUESTÃO {questionIndex + 1}</span>
                    {isLoadingQuestion && <div className="w-4 h-4 border-2 border-primary border-t-transparent rounded-full animate-spin"></div>}
                </div>
            </header>
            <main className="flex-1 overflow-y-auto px-6 pt-2 pb-32">
                <div className="mb-8">
                    ...
                    {currentQuestion.baseText && (
                        <div className="mb-4 p-4 bg-slate-100 dark:bg-slate-800/50 border-l-4 border-blue-600 dark:border-blue-400 rounded-lg max-h-40 overflow-y-auto italic">
                    ...
                    {currentQuestion.imageUrl2 && (<img src={currentQuestion.imageUrl2} className="w-full h-auto rounded-lg my-4 shadow-sm border border-slate-200 dark:border-slate-800" alt="Ilustração da questão (parte 2)" />)}
                </div>
            ...
            {showTutor && <AiTutorModal explanation={aiExplanation} onClose={() => loadNewQuestion(selectedSubject!)} />}
        </div>
    );
};
